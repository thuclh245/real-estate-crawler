...
def publish_gcs_to_bq(
    folder_name: str, table_id: str, partition_field: str = "snapshot_date", is_partitioned: bool = True
) -> bool:
    """
    ...
    if is_partitioned:
        # Load only parquet files from subdirectories to avoid matching root metadata files like _SUCCESS
        gcs_uri = f"gs://{bucket_name}/gold/{folder_name}/*/*.parquet"
        ...
        # Load flat parquet files from the root of the folder
        gcs_uri = f"gs://{bucket_name}/gold/{folder_name}/*.parquet"
...
def main():
    ...
    for folder_name, table_id in tables.items():
        ...
        success = publish_gcs_to_bq(folder_name, table_id, partition_col, is_partitioned=is_partitioned)
        if success:
            success_count += 1
