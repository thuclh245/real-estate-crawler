...
def publish_gcs_to_bq(
    folder_name: str, table_id: str, partition_field: str = "snapshot_date"
) -> bool:
    """
    Loads hive-partitioned Parquet files from Google Cloud Storage into BigQuery.
    Uses Hive Partitioning to dynamically reconstruct partition columns (snapshot_date / crawl_date)
    so that BigQuery can successfully align partition fields that are not embedded inside the Parquet bytes.
    """
    try:
    ...
    print(f"\n[BQ PUBLISH] Loading partitioned folder from GCS: {gcs_uri}...")
...
def main():
    ...
    for folder_name, table_id in tables.items():
        ...
        success = publish_gcs_to_bq(folder_name, table_id, partition_col)
        if success:
            success_count += 1
