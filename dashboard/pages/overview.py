...
def render_overview(
    current_df: pd.DataFrame, snapshot_df: pd.DataFrame, quality_df: pd.DataFrame
) -> None:
    st.subheader("Pipeline Overview")

    summary = read_phase3_summary()
    latest_quality = (
        quality_df.sort_values("crawl_date").tail(1).iloc[0]
    ...
    total_records = int(
        summary.get("total_silver_records", latest_quality.get("total_records", 0))
    )
    duplicate_rate = summary.get(
        "duplicate_rate", latest_quality.get("duplicate_rate", None)
    )
    parse_success_rate = summary.get(
        "parse_success_rate", latest_quality.get("parse_success_rate", None)
    )
    missing_price_rate = summary.get(
        "missing_price_rate", latest_quality.get("missing_price_rate", None)
    )
    missing_area_rate = summary.get(
        "missing_area_rate", latest_quality.get("missing_area_rate", None)
    )
    missing_location_rate = summary.get(
        "missing_location_rate", latest_quality.get("missing_location_rate", None)
    )
    ...
    c8.metric(
        "Snapshot Dates",
        (
            snapshot_df["snapshot_date"].nunique()
    ...
    with left:
        if "snapshot_status" in snapshot_df.columns:
            ...
            fig = px.pie(
                status_counts,
                names="snapshot_status",
                values="count",
                title="Snapshot Status Distribution",
            )
    ...
    with right:
        if "property_type_group" in current_df.columns:
            property_counts = (
                current_df["property_type_group"]
                .fillna("unknown")
                .value_counts()
                .reset_index()
            )
