...
from crawl import (  # noqa: E402
    ...
    parse_detail_page_location_fields,
)
from crawl_audit import (  # noqa: E402
    audit_location,
    classify_category_match,
    safe_rate,
    write_audit_sample_csv,
)
from parser import html_to_text  # noqa: E402
from storage import save_json_file  # noqa: E402
...
def find_crawl_root(crawl_id: str) -> Path:
    ...
    if not matches:
        raise FileNotFoundError(f"Cannot find Bronze crawl root for crawl_id={crawl_id}")
    if len(matches) > 1:
        raise RuntimeError(f"Multiple crawl roots found for crawl_id={crawl_id}: {matches}")
    return matches[0]
...
def build_seed_audits(crawl_root: Path) -> list[dict]:
    seed_audits = {}
    for debug_json_path in sorted((DEBUG_LIST_ROOT / f"crawl_id={crawl_root.name.removeprefix('crawl_id=')}").glob("*.json")):
        record = load_json(debug_json_path)
        page_url = record.get("page_url")
        if not page_url:
            continue
        seed_audits.setdefault(page_url, {
            "seed_url": page_url,
...
def backfill_crawl(crawl_id: str) -> dict:
    ...
    for metadata_path in metadata_paths:
        ...
        title = detail_fields.get("detail_title") or listing_card.get("listing_card_title") or metadata.get("title")
        description = detail_fields.get("detail_description") or listing_card.get("listing_card_description") or metadata.get("description")
        target = {
            "location_label": metadata.get("crawl_location_label") or metadata.get("crawl_district_label"),
            "location_slug": metadata.get("crawl_location_slug") or metadata.get("crawl_district"),
            "location_path": metadata.get("crawl_location_path") or metadata.get("crawl_district"),
        ...
        updates = {
            ...
            "listing_card_old_district_raw": listing_card.get("listing_card_old_district_raw"),
        ...
        detail_audit = {
            ...
            "listing_url": listing_url,
            "source_seed_url": metadata.get("source_seed_url") or metadata.get("crawl_seed_url"),
            ...
            "listing_card_old_district_raw": metadata.get("listing_card_old_district_raw"),
    ...
    valid_seed_urls = sum(1 for record in seed_audits if record.get("is_seed_url_valid") is True)
...
def main():
    parser = argparse.ArgumentParser(description="Backfill location/category audit fields for an existing Bronze crawl.")
