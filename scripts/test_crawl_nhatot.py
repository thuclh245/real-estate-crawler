...
def parse_listing_rows(
    ...
    for index, ad in enumerate(ads, start=1):
        ...
        row = {
            ...
            "area_m2": ad.get("size"),
            "area_raw": clean_text(ad.get("size_unit_string")) or (
                f"{ad.get('size')} m2" if ad.get("size") is not None else None
            ),
            "bedroom_count": ad.get("rooms"),
            "bathroom_count": ad.get("toilets"),
            "floor_count": ad.get("floornumber"),
            "property_subtype_raw": clean_text(param_value(ad, "apartment_type") or param_value(ad, "house_type")),
    ...
    async with AsyncWebCrawler(verbose=False) as crawler:
        for district in selected_districts:
            for category in selected_categories:
                ...
                print(f"[FETCH] {district['name']} | {category['name']} | page={requested_page} | {page_url}")
                ...
                try:
                    ...
                    if not rows and args.fallback_to_page_one and requested_page != 1:
                        ...
                        print(f"[FALLBACK] no listings on page={requested_page}; trying page=1 | {fallback_url}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--page", type=int, default=None, help="Explicit page number. Defaults to run-date page cycle.")
    parser.add_argument("--page-cycle", type=int, default=5, help="Rotate pages by day. 2026-05-19 maps to page 4.")
    parser.add_argument("--run-date", default=None, help="YYYY-MM-DD date used for page rotation. Defaults to today.")
