...
def parse_listing_cards(
    ...
    for index, card in enumerate(soup.select("article.property-item"), start=1):
        ...
        title = clean_text(card.select_one(".property-title").get_text(" ", strip=True)) if card.select_one(".property-title") else None
        ...
        row = {
            ...
            "title_raw": title,
            "description_raw": clean_text(card.select_one(".brief").get_text(" ", strip=True)) if card.select_one(".brief") else None,
            "price_raw": clean_text(card.select_one(".price").get_text(" ", strip=True)) if card.select_one(".price") else None,
            "area_raw": clean_text(card.select_one(".area").get_text(" ", strip=True)) if card.select_one(".area") else None,
            "street_width_raw": clean_text(card.select_one(".street-width").get_text(" ", strip=True)) if card.select_one(".street-width") else None,
            "floor_count_raw": clean_text(card.select_one(".floors").get_text(" ", strip=True)) if card.select_one(".floors") else None,
            "bedroom_count_raw": clean_text(card.select_one(".bedroom").get_text(" ", strip=True)) if card.select_one(".bedroom") else None,
            "parking_raw": clean_text(card.select_one(".parking").get_text(" ", strip=True)) if card.select_one(".parking") else None,
            "new_address_raw": clean_text(card.select_one(".new-address").get_text(" ", strip=True)) if card.select_one(".new-address") else None,
            "old_address_raw": clean_text(card.select_one(".old-address").get_text(" ", strip=True)) if card.select_one(".old-address") else None,
...
async def run(args: argparse.Namespace) -> Path:
    ...
    page_summaries = []
    all_rows = []
    group_count = min(args.max_districts, len(DISTRICTS)) * min(args.max_categories, len(CATEGORIES))
